    ...
    while (std::getline(file, line))
    {
        ...
        if (line.starts_with("Kd "))
        {
           ...
           data.materials[currentMaterialIndex].diffuseColor = {R, G, B, 255.0f};
            std::cout << "DiffuseColor: " << "R: " << data.materials[currentMaterialIndex].diffuseColor.red << " G: " <<
            data.materials[currentMaterialIndex].diffuseColor.green << " B: " << data.materials[currentMaterialIndex].diffuseColor.blue << "\n";
        }
        ...
        if(line.starts_with("map_Kd "))
        {
            ...
            std::cout
            << "Texture path: "
            << data.materials[currentMaterialIndex].diffuseTexturePath
            << '\n';
        }
    }
    return data;
